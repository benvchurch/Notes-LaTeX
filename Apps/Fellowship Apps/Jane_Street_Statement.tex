...
\fancyhead[RH]{\textbf{Research Plan}}
...
\textbf{Introduction:}
An algebraic variety $X$ is \textit{unirational} if there exists a dominant rational map $\P^n \rat X$ and \textit{rational} if this map can be made an birational equivalence. In 1875, J. L\"{u}roth showed \footfullcite{Luroth} that rationality and unirationality coincide for algebraic curves. The classical L\"{u}roth problem asks if rationality and unirationality are equivalent for higher dimensional varieties. This question led to enormous progress in understanding algebraic surfaces, which culminated in the Enriques-Kodaira classification of algebraic surfaces.
