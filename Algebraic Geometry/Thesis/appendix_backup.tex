...
\section{Pic Functors}


\begin{question}
In this section I did everything for $k = \mathbb{C}$ since this is what is given in Harris-Mumford. Can we extend this to other fields?
\end{question}

\begin{question}
I assumed that the toric surface $S$ is smooth here for two reasons. (1) to ensure that $\sPic{X/k}$ exists and (2) such that $C \embed S$ is Cartier. Can we drop this assumption?
\end{question}

Here, we investigate the behavior of very general curves with respect to embeddings onto smooth toric surfaces. Our result is that for sufficiently large genus, very general curves cannot embed in any smooth toric surface. Intuitively, a very general curve is a curve the coefficients of whose defining equations do not satisfy any algebraic relations over $\Q$. Specifically, we define a very general curve as follows.
...
\noindent
To prove the required result, we will make use of the following theorem of Harris and Mumford which restricts the birationality classes of surfaces on which nontrivial families of very general curves lie.


\begin{theorem}[Harris-Mumford]
Let $C$ be a generic curve of genus $g \ge 23$ and $S$ an algebraic surface containing $C$ such that $C$ moves in a nontrivial linear system on $S$ meaning that $\dim H^0(S, \struct{S}(C)) > 1$. Then $S$ is a ruled surface birational to $C \times \P^1$. 
\end{theorem}
...
and $\mathrm{Pic}_{X/S}^{\et}$ the associated sheaf for the \etale topology. If it exists, the Picard scheme $\sPic{X/S}$ is the unique scheme representing this sheaf,
...
\end{theorem}
\noindent
In particular, when $f : X \to \Spec{k}$ is a projective geometrically integral variety over $k$ then the Picard scheme $\sPic{X/k}$ exists. 
The topology of the Picard scheme is related to a powerful equivalence relation on line bundles known as algebraic equivalence which is the algebraic version of topological homotopy equivalences of bundles.
...
\begin{prop}
Let $\csPic{X/k} \embed \sPic{X/k}$ be the connected component of the identity. Then $\csPic{X/k}(k)$ is exactly the group of line bundles $\L \in \mathrm{Pic}^0(X)$ algebraically equivalent to zero. 
\end{prop}
...
\begin{rmk}
From the above discussion, we can define the Neron-Severi group $\NS{S}$,
\begin{center}
\begin{tikzcd}
0 \arrow[r] & \mathrm{Pic}^0(X) \arrow[r] & \Pic{X} \arrow[r] & \NS{S} \arrow[r] & 0 
\end{tikzcd}
\end{center}
Therefore, the Neron-Severi group is the group of line bundles modulo algebraic equivalence or equivalently the group of connected components of $\sPic{X/k}$.
\end{rmk}

\begin{theorem}
Let $X \to \Spec{k}$ and assume that $\Pic{X/k}$ exists representing $\mathrm{Pic}_{X/l}^{\et}$. Then the Zariski tangent space at the trivial bundle has a canonical identification,
\[ T_0 \sPic{X/k} = H^1(X, \struct{X}) \]
thus $\dim{\sPic{X/k}} \le \dim_k H^1(X, \struct{X})$ with equality exactly when $\sPic{X/k}$ is smooth at $0$. Since $\sPic{X/k}$ is a group scheme, in  this case $\sPic{X/k}$ is everywhere smooth of dimension $\dim_k H^1(X, \struct{X})$.
\end{theorem}
...
\begin{rmk}
For a smooth proper curve $C$ over $k$ of genus $g$. Then $T_0 \sPic{X/k} = H^1(X, \struct{X})$ has dimension $g$ so $\csPic{X/k}$ is an abelian variety of dimension $g$ which is the Jacobian variety of $C$. 
\end{rmk}
