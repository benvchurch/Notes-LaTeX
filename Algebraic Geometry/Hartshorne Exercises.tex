...
since $f^{-1}(V) \supset U$ gives restriction maps $\F(f^{-1}(V)) \to \F(U)$ compatilbe with restricton. Sheafifying gives a natural map $f^{-1} f_* \F \to \F$. Furthermore, 
(FINSIH) 
...
\end{lemma}
\begin{proof}
Consider $f(V(I)) \subset V(J)$ then $I \subset \varphi^{-1}(\P)$ for each prime ideal $\P \subset B$ above $I$ so $\varphi(J) \subset \sqrt{J}$. By the above lemma, $J \subset \varphi^{-1}(\sqrt{I}) = \sqrt{\varphi^{-1}(I)}$ and thus,
...
\end{tikzcd}
\end{center}
Then since the functor $(-)|_U$ is exact and $j_*$ is left-exact then we get a commutative diagram with exact rows,
\begin{center}
\begin{tikzcd}
...
\\
0 \arrow[r] & H^0(\U, \F) \arrow[r] & H^0(\U, \F) \arrow[r] & H^0(\U, \K) \arrow[r] & H^1(\U, \F) \arrow[r] & 0
\end{tikzcd}
\end{center}
