...
\end{equation}
For stars oscillating through the disc with vertical displacement large compared to the scale height the period of oscillation is simply given by approximating the the disc gravity as producing a constant magnitude restoring force. Therefore the period for a star with vertical velocity $\sigma_D$ is given approximately by,
\begin{equation}
P = \frac{2 H}{\sigma_D} = \frac{2 \sigma_D}{4 \pi G \Sigma}.
\end{equation}
The period of disc oscillations determines the adiabatic cutoff for heating due to perturbing interactions. Under the assumption that disc thickening is due primarily to the proposed dark substructure dynamics, the dependence on radius of the disc velocity dispersion $\sigma_D$ can be calculated explicitly. Using the above relations and the radial dependence of $\sigma_D$ we can predict the radial thickness profile of the galactic disc and compare these results to measurements of the flaring of actual discs.  
\par
...
\begin{equation}
\ln{\Lambda} \equiv \ln{\left( \frac{b_{\text{max}}}{b_{\text{min}}} \right)},
\end{equation}
...
The maximum distance across which the heating is efficient, $b_{\max}$, is fixed by equation \eqref{adiabatic} and the minimum distance, $b_{\min}$, is given by the characteristic size for the perturbing objects denoted, $r_{l}$. Furthermore, the period of oscillation out of a thin disc is given by the velocity dispersion and surface density of the disc via,
\begin{equation}
...
\deriv{\sigma_D^2}{t} = \frac{8 \pi G^2}{\sqrt{2} \: \sigma_H} \int_0^{M} T_R(m)^2  \deriv{n}{m}  \ln{\frac{\sigma_H \sigma_D}{4 \pi G \Sigma \: r_{\text{t}}(m)}} \d{m},
\end{equation}   
...
\begin{align}
\deriv{\sigma_D^2}{t} & = \frac{2 \pi A}{3 \sqrt{3}} \left( \frac{2 \pi \hbar }{m_a} \right)^3 \frac{(\rho(r) G)^2}{\sigma_H^4} \ln{\Lambda} ;
\\
\Lambda & = \frac{m_a \sigma_H^2 \sigma_D}{4 \pi \hbar G \Sigma} .
\end{align}
\end{subequations}
...
\vspace*{-5mm}
\caption{Rate of heating due to wavelets at $r = 8\, \si{\kilo \parsec}$ as a function of axion masses $m_{a}$ in the range \SIrange{0.1 e-22}{ 10 e-22}{\electronvolt} for a Milky Way-like halo. The horizontal line is fixed at $32\, \si{\kilo\meter\per\second}$, the observational maximum on velocity dispersion of the Milky Way thick disc. }
...
\[ m_a > 0.65 \times \SI{e-22}{\electronvolt} \]
under the assumption that all disc heating is reflected in the z-velocity dispersion.
...
We find that the heating of Milky Way type stellar discs by substructure gravitational fluctuations will lead to flaring and disc destruction at radii greater than \SIrange{15}{20}{\kilo\parsec} in both CDM and FDM paradigms. However, quantum wavelets in FDM can be effective at heating the inner disc regions accounting for the thick disc and the observed flaring of star populations. A particle mass in excess of $m_a > \SI{0.65 e-22}{\electronvolt}$ is required to avoid exceeding observational bounds on heating of thick disc stars in the Milky Way. Correspondingly, an FDM scenario with particle mass $m_a \approx \SI{e-22}{\electronvolt}$ can, in fact, explain the observed increase in velocity dispersion with increasing stellar age observed in the solar neighborhood.  
\par
