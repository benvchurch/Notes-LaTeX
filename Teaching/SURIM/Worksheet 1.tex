...
Problems marked with $*$ are very hard or use more advanced techniques and ideas. Feel free to move on without fully solving them. Make sure to read and digest the statements since they may be useful (as a black box) in future problems.
...
\[ f(r_0(t_1, \dots, t_n), \dots, r_n(t_1, \dots, t_n)) = 0 \]
...
\subsection{} \label{character_counting_roots}
...
Let $X$ be a variety over $\FF_q$. Then for any $k$ we can ``base change'' it to a variety $X'$ over $\FF_{q^k}$. If $X$ is defined by an equation $f \in \FF_q[x_0, \dots, x_n]$ then $X'$ is simply defined by the same polynomial $f$ but viewed as an element of $\FF_{q^k}[x_0, \dots, x_n]$. Explicitly, $X'$ is characterized by the property that,
...
Show that the number $N(u)$ of solutions to $x^n = u$ defined in Problem~\ref{character_counting_roots} can be expressed as,
...
\[ j(\alpha) = \frac{1}{q} g(\chi_{\alpha_0}) \cdots g(\chi_{\alpha_r}) \]
are all roots of unity. This is a difficult problem in general. However, it can be turned into entirely combinatorics via the theorem of Stickelberger [Lang's Algebraic Number Theory, IV.4, Theorem 10 (p.97)]. 
...
\item[$(b)*$] $r = 3$ and $p,q,w$ be primes such that $p,q,w \equiv 1 \mod s$ for some integer $s$ and let $X$ be defined by,
...
\begin{enumerate}
\item \chref{http://www.ams.org/journals/bull/1949-55-05/S0002-9904-1949-09219-4/S0002-9904-1949-09219-4.pdf}{Weil's original paper}
\item \chref{https://projecteuclid.org/journals/tohoku-mathematical-journal/volume-31/issue-1/On-Fermat-varieties/10.2748/tmj/1178229881.full}{Shioda on Fermat varieties}
...
At this point, we will start looking for new examples. Remind me to send you notes on the relationships between diagonal varieties and tricks for computing the sums that arise from Stickelberger's theorem. 
