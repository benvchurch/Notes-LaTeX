...
The first is \chref{https://stacks.math.columbia.edu/tag/0FD2}{Tag 0FD2} (recall that $k$ is perfect is $X$ is geometrically reduced). The second is as follows. Since $X$ is integral and proper $k' = \Gamma(X, \struct{X})$ is a finite $k$-algebra domain and hence a finite field extension of $k$. In particular, $k' \subset K \cap \bar{k}$ inside $\overline{K}$ proving one direction. Furthermore, suppose that $f \in K$ is integral over $k'$. Then for any affine open $U \subset X$ we see that $f$ is integral over $\struct{X}(U)$ and hence $f \in \struct{X}(U)$ since $X$ is normal so $\struct{X}{U}$ is a normal domain with fraction field $K$. Hence since $f \in \struct{X}(U)$ for each affine open $f \in \Gamma(X, \struct{X})$ proving that $k'$ is algebraically closed inside $K$. 
\end{rmk}
