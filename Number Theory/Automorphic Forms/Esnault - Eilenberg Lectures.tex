...
\\
& \Gamma^{\alg} \arrow[u, dashed, two heads] \arrow[r, equals] & \varprojlim\limits_{\varphi : \Gamma \to \GL_r(\CC)} \im{\varphi} \arrow[u, dashed]
\end{tikzcd}
\end{center}
...
\begin{theorem}[Shusterman-Swnivas]
...
for a fixed number field so there is a lattice $E^r$ such that $\tho_{\LL_\ell} : \pi_1(X_{\overline{\FF}_q, x) \to \GL_r(\struct{E})$. Since $\sigma(\struct{E}) = \struct{E}$ the companion, if it exists, are defined over $A$ for $A \embed \Qbar_{\ell'}$ is the completion at a place over $\ell'$. 
