...
\end{tikzcd}
\end{center}
Then pulling back we get the left-exact sequence,
\begin{center}
\begin{tikzcd}
...
\begin{rmk}
For any $\struct{S}(S)$-algebra $A$, we know that $\Hom{S}{X}{\Spec{A}} = \Hom{\struct{S}(S)}{A}{\Gamma(X, \struct{X}}$. We want to generalize this to $\struct{X}$-algebras $\sA$.
\end{rmk}
...
\[ \rank_y{(f_* \struct{X})} = \dim_{\kappa(y)} \left( (f_* \struct{X}) \otimes_{\stalk{X}{x}} \kappa(y) \right) \]
...
\begin{defn}
A morphism $f : X \to Y$ is \textit{finite locally free} if $f_* \struct{X}$ is a finite locally free as a $\struct{Y}$-module.
\end{defn}
...
\begin{proof}
It suffices to show that if $A \to B$ is finite then $B$ is locally free iff it is flat and finitely presented an $A$-module. We know that finite locally free implies flat and locally finitely presented\footnote{it is finitely presented as an $A$-algebra because it is finitely presented as an $A$-module } (thus finitely presented). Conversely if $B$ is flat and finitely presented then it is projective (see Tag 00NX) and hence locally free. 
\end{proof}
